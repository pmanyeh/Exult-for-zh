...
*   `<font_path>`：**主要中文字型路徑 (預設值: `<PATCH>/chinese.ttf`)**
    *   說明：指定遊戲要使用的主要 TTF 字型檔路徑。若未指定，會自動尋找 Patch 資料夾下的 `chinese.ttf`。
*   `<small_font_path>`：**小字體專用字型路徑**
    *   說明：當字級小於等於 10 的時候（例如書本、捲軸的微小文字），會優先使用此字型檔。如果未設定此參數或找不到指定的檔案，則會自動退回使用 `<font_path>` 指定的一般字型。
...
*   `<font_size_bark>`：**頭頂與標籤字型大小 (預設值: `15`)**
    *   說明：專門控制點擊物件時浮現的名稱、NPC 在地圖上走動時頭頂彈出的對話氣泡大小、目前法術書書籤指定法術名稱的大小。若未設定，將繼承對話字型的大小。其餘陰影與間距參數則與上述「NPC 對話」共用。
...
<config>
  <video>
    <chinese>
      <!-- 全域設定 -->
      <font_path> <PATCH>/chinese.ttf </font_path>
      <small_font_path> <PATCH>/chinese_small.ttf </small_font_path>
